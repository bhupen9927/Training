(function () {
  try {
            ...
            <ul class="eg-courses_container" data-studyarea="${area.studyarea}">
              ${area.courses.map(course => `
                <li class="${course.id}" ${course.isAvailable === false ? 'isavailable="false"' : 'isavailable="true"'}>
                  ${course.courseUrl ? `<a href="${course.courseUrl}">${course.coursename}</a>` : `<span>${course.coursename} <div class="eg-dropdown"><span>Show specialist studies</span></div></span>`}
    ...
    function init() {
      ...
      document.querySelector('#menu-main-menu > li:first-of-type > .sub-menu').outerHTML = megamenuhtml;
