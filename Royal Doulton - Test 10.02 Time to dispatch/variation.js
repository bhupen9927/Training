(function () {
    try {
        ...
        // Function to check and update SVG colors
        function updateSVGColors() {
            var productBackgroundColor = getComputedStyle(document.body).getPropertyValue('--product-background-color').trim();
            var svgIcon = document.querySelector('.svg-icon-size');
            if (productBackgroundColor === '#00313c') {
                svgIcon.style.fill = 'white';
                svgIcon.style.stroke = 'white';
            } else {
                svgIcon.style.fill = 'black';
                svgIcon.style.stroke = '#0000004a';
            }
        }
        ...
        function init() {
            ...
            var observer = new MutationObserver(updateSVGColors);
